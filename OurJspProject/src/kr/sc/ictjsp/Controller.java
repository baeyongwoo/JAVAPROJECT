...
public class Controller extends HttpServlet {
	...
	protected void deRequest(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		// businessUserService
		...
		if (uri.equals("/OurJspProject/join.use")) {
			...
			busv = new UserGetinfoService();
			busv.execute(request, response);
			String session1 = (String) request.getAttribute("updateUser");
			System.out.println(session1);
			if (session == null && session1.equals("success")) {
				ui = "/UserView/login_form.jsp";
			}else {
				ui = "/UserView/login_success.jsp";
			}
